...
export default async function AcademyPage() {
  ...
  const isFoundationItem = (item: (typeof items)[number]) =>
    item.slug === 'about-line-gift' || item.typeFields.publicPath === publicRoutes.aboutLineGift
  const foundationItem = items.find(
    (item) => isFoundationItem(item) && parseLineGiftOfficialFields(item.typeFields),
  )
  const articleItems = items.filter((item) => !isFoundationItem(item))
  return (
    <>
      ...
      <Container className="py-16 sm:py-20">
        ...
        <div className="mt-10">
          {articleItems.length === 0 ? <p className="rounded-card border border-line bg-paper p-6 text-muted">文章將在內容通過來源與年份查核後發布。</p> : <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{articleItems.map((item) => <ContentCard key={item.id} href={`/line-gift-academy/${item.slug}`} title={item.title} excerpt={item.excerpt} eyebrow="LINE 禮物研究院" publishedAt={item.publishedAt} />)}</div>}
        </div>
      </Container>
