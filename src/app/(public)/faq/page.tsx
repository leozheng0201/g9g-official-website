...
export default async function FaqPage() {
  const published = await listPublishedContent({ contentType: 'faq' })
  const items = published.map((item) => ({ id: item.slug, question: String(item.typeFields.question ?? item.title), answer: item.excerpt ?? '' }))
  return (
    <>
      <PageHero eyebrow="FAQ" title="常見問題" description="先把平台、費用、合作與品牌成長健檢常見問題說清楚。" />
      <Container className="py-16 sm:py-20">
        <Breadcrumbs items={[{ label: '常見問題' }]} />
        <div className="mx-auto max-w-4xl"><Accordion items={items} /></div>
      </Container>
