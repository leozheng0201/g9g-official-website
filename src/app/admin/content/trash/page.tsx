...
export default async function ContentTrashPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10">
      ...
      <p className="mt-3 text-muted">內容保留 30 天。還原後回到草稿；永久刪除仍會由伺服器再次檢查角色與保留期限。</p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {role === 'super_admin' ? (
                    <form action={permanentlyDeleteContentAction}>
                      <input type="hidden" name="id" value={item.id} />
                      <button className="rounded-full border border-red-300 px-4 py-2 text-sm font-black text-red-700" type="submit">永久刪除（滿 30 天後）</button>
                    </form>
                  ) : null}
                </div>
              </div>
