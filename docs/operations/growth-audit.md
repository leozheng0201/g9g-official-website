...
GROWTH_AUDIT_FINGERPRINT_SECRET=
```

要求：

- `SUPABASE_SERVICE_ROLE_KEY`、`RESEND_API_KEY`、`GROWTH_AUDIT_FINGERPRINT_SECRET` 只能存在 Vercel server environment，不得使用 `NEXT_PUBLIC_` 前綴。
- `GROWTH_AUDIT_FINGERPRINT_SECRET` 使用至少 32 bytes 的隨機值；Preview 與 Production 使用不同值。
