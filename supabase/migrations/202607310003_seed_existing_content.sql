do $$
declare
  smile_id uuid := '30000000-0000-4000-8000-000000000001';
  fab_id uuid := '30000000-0000-4000-8000-000000000002';
  item_id uuid;
  faq record;
  snapshot jsonb;
begin
  ...
  (
    smile_id, 'case_study', '微笑甜果｜LINE 禮物電商案例', 'smile-fruit',
    ...
    '微笑甜果 LINE 禮物電商案例',
    '了解微笑甜果如何以送禮情境與主打品策略建立節慶檔期成長。',
    'published', 10, now(), 1
  ),
  (
    fab_id, 'case_study', '法布甜｜整體電商與品牌轉型案例', 'ar-patisserie',
    ...
    '法布甜整體電商與品牌轉型案例',
    '了解法布甜如何從傳統伴手禮市場升級至高附加價值送禮市場。',
    'published', 20, now(), 1
  )
  on conflict (id) do nothing;

  for item_id in select unnest(array[smile_id, fab_id]) loop
  ...
  for faq in
    select * from (values
      ('fit','什麼品牌適合進入 LINE 禮物？','會從穩定供貨、基本客服、送禮潛力，以及品牌是否願意建立主打商品一起判斷。',10),
      ('before-entry','還沒進駐 LINE 禮物，可以先合作嗎？','可以先做品牌成長健檢與商城啟動規劃，但 G9G 不保證平台審核結果。',20),
      ('guarantee','G9G 會保證上架或業績嗎？','不會。G9G 不保證商城核准、特定業績、流量或搜尋排名。',30),
      ('placements','廣告與站內版位費包含在服務費嗎？','廣告預算與 LINE 禮物站內版位費另計；G9G 依合作方案提供策略、規劃與基本版位素材尺寸延伸。',40),
      ('plans','三種代營運方案有什麼差異？','商城啟動適合第一次進場；成果分潤依完成訂單成交額計算；全年代營運建立 12 個月的商品、檔期、版位與數據節奏。',50),
